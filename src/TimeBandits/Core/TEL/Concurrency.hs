...
import TimeBandits.Core.TEL.AST (Expression(..), Program(..), TypeExpr(..))
import TimeBandits.Core.ProgramId (ProgramId)
import TimeBandits.Core.Concurrency.Types (EffectType)
import TimeBandits.Core.Effect (Effect)
import TimeBandits.Core.ResourceId (ResourceId)
import TimeBandits.Core.Error (Result)
import TimeBandits.Core.Concurrency.Combinators
    ( Condition(..)
    , ResourceCondition(..)
    )
